...
IMAGE_SIZE = (84, 84)
ACTION_DIM = 4
STEP_SIZE = 0.01     # how much EE moves per keypress
# -----------------------------

def get_action_from_keys(keys):
    """
    Maps keys to 4-dim action:
    [dx, dy, dz, gripper]
    gripper: +1 open, -1 close, 0 unchanged
    """
    dx = dy = dz = 0
    grip = 0
...
def get_camera_image():
    ...
    rgb = np.reshape(px, (height, width, 4))[:, :, :3]  # drop alpha
...
print("\nCollecting demonstrations. Press ESC to quit.\n")
...
while True:
    ...
    for j in range(7):
        p.setJointMotorControl2(
            robot, j, p.POSITION_CONTROL, joint_angles[j]
        )
